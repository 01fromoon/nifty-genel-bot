...
const GLOBAL_COOLDOWN_SEC = 5; 

function isPremiumUser(userId) {
  const data = db.get(`premiumUser.${userId}`);
  return !!data; 
}
...
export default class Bot extends Client {
  ...
  constructor(options = {}) {
    ...
    const InteractionHandler = async (interaction) => {
      if (
        ...
        if (cmd) {
          ...
          const isPremium = isPremiumUser(interaction.user.id);
          if (!isPremium) {
            const gCooldownKey = `${interaction.user.id}`;
            const now = Date.now();
            if (globalCooldowns.has(gCooldownKey)) {
              const last = globalCooldowns.get(gCooldownKey);
              if (now - last < GLOBAL_COOLDOWN_SEC * 1000) {
                const kalan = ((GLOBAL_COOLDOWN_SEC * 1000 - (now - last)) / 1000).toFixed(1);
                return interaction.reply({
                  content: `⏳ Lütfen biraz bekle! Tüm komutlarda **${GLOBAL_COOLDOWN_SEC} saniye** global bekleme süresi var. Kalan: **${kalan} saniye**\n\n🚀 Bekleme süresinden muaf olmak ve premium ayrıcalıklara ulaşmak için **/premium** komutunu deneyebilirsin!`,
                  ephemeral: true,
                });
              }
            }
            globalCooldowns.set(gCooldownKey, now);
            setTimeout(() => globalCooldowns.delete(gCooldownKey), GLOBAL_COOLDOWN_SEC * 1000);
          }
    ...
    this.once("ready", () => {
      const activities = [
        { name: "✨ 01fromoon tarafından yapıldı!", type: 0 },
        { name: "💡 /yardım   ⚙️ /ayarlar", type: 0 },
        {
          name: () => `🌐 ${this.guilds.cache.size} sunucu, 👤 ${this.users.cache.size} kullanıcı, #️⃣ ${this.channels.cache.size} kanal`,
          type: 0
        }
      ];
      let i = 0;
...
try {
  this.user.setActivity(name, { type: a.type });
} catch (e) {
}
